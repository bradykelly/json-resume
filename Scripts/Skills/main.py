...
def main(args: List[str]) -> str | JsonResumeBaseModel:
    ...
    :return: A JSON string if the command is `serialize`, or a JsonResumeBaseModel object if the command is `deserialize`.
    """

    if len(args) < 2:
        skill_serializer = SkillsSerializer()
        skills = read_skills_csv(args[1])
        json = skill_serializer.serialize_skills(skills)
        return json


def read_skills_csv(csv_path: str) -> Skills:
    """
    ...
    if len(csv_path) > 0 and os.path.isfile(csv_path):
        try:
            with open(csv_path, 'r') as file:
                ret_list: List[str] = file.readlines()
                return ret_list
        except IOError as e:
